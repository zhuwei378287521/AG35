...
retry:
	...
	if (err) {
		...
		if (mtd_is_bitflip(err)) {
			...
			if(len != read)
			{
				...
				//按照加载顺序，Linux的system加载时是 ubi 0， 所以这里限制一下，避免modem ubi 加载出现这个错误也会进入这里。
					...
					{
						printk("@Ramos set restore systemfs flag here 9999\r\n");
						Quectel_Set_Partition_RestoreFlag("system",9);
					}else{
						Quectel_Set_Partition_RestoreFlag("recovery",9);
					}
				}
		...
		//按照加载顺序，Linux的system加载时是 ubi 0， 所以这里限制一下，避免modem ubi 加载出现这个错误也会进入这里。
			if (!get_bootmode(NULL))
			{
				printk("@Ramos set restore systemfs flag here 5555\r\n");
				Quectel_Set_Partition_RestoreFlag("system",5);
			}else{
				Quectel_Set_Partition_RestoreFlag("recovery",5);
			}
		}
#endif
